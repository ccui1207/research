...
"""
	Example usage:
		python APKScanner_ReportByVectorKey.py -v [vector_name] -m [mode] -l [Log level]
		python APKScanner_ReportByVectorKey.py -v WEBVIEW_RCE -m massive -l Critical
...
def parseArgument():
	parser = argparse.ArgumentParser(description='APKScanner Framework: Android APK Vulnerability Reporter by Vector Name')
...
print("## APKScanner Framework: Android APK Vulnerability Reporter by Vector Name ##")
print()

db_config_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'androbugs-db.cfg')

if not os.path.isfile(db_config_file) :
	print(("[ERROR] APKScanner Framework DB config file not found: " + db_config_file))
	traceback.print_exc()
