...
---
2. 先准备本地测试后端
测试 Naxsi 前，需要一个后端服务供 Nginx 反向代理。
2.1 后端目录
```bash
...
```
如果看到 `nginx` 监听 `0.0.0.0:8081`，说明 Naxsi 测试站点已经运行。
---
5. 基础联通测试
5.1 正常请求测试
```bash
...
规则 ID：`1303`
也就是说：
Naxsi 已经成功识别异常参数，只是当前处于 LearningMode，所以还未拦截。
---
8. 切换到真实拦截模式
编辑站点配置：
```bash
...
```
这一步说明：
Naxsi 已从“记录模式”切换到“真实拦截模式”。
---
9. 批量 fuzz 测试（防模糊攻击能力测试核心）
在 Naxsi 源码目录中创建脚本：
```bash
cd ~/research/防止模糊攻击/naxsi
```
创建 `fuzz_simple.py`：
```python
import random
